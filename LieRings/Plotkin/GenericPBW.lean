...
/-!
# Generic ordered PBW for a based Lie algebra
...
private def DegreeLE (d : ℕ) (p : MvPolynomial ι R) : Prop :=
  ∀ e ∈ p.support, exponentDegree ι e ≤ d

private theorem degreeLE_iff_totalDegree_le (d : ℕ) (p : MvPolynomial ι R) :
...
private theorem degreeLE_zero (d : ℕ) :
    DegreeLE ι d (0 : MvPolynomial ι R) := by
  simp [DegreeLE]

private theorem degreeLE_add {d : ℕ} {p q : MvPolynomial ι R}
...
private theorem degreeLE_sub {d : ℕ} {p q : MvPolynomial ι R}
    (hp : DegreeLE ι d p) (hq : DegreeLE ι d q) : DegreeLE ι d (p - q) := by
  rw [degreeLE_iff_totalDegree_le] at hp hq ⊢
  exact (MvPolynomial.totalDegree_sub p q).trans (max_le hp hq)

private theorem degreeLE_smul {d : ℕ} (r : R) {p : MvPolynomial ι R}
...
private theorem degreeLE_neg {d : ℕ} {p : MvPolynomial ι R}
    (hp : DegreeLE ι d p) : DegreeLE ι d (-p) := by
  rw [degreeLE_iff_totalDegree_le] at hp ⊢
  simpa using hp

private theorem degreeLE_mono {d e : ℕ} {p : MvPolynomial ι R}
    (hde : d ≤ e) (hp : DegreeLE ι d p) : DegreeLE ι e p := by
  exact fun a ha ↦ (hp a ha).trans hde

private theorem degreeLE_mul {d e : ℕ} {p q : MvPolynomial ι R}
...
private theorem degreeLE_monomial (e : ι →₀ ℕ) (r : R) :
...
private theorem degreeLE_X (i : ι) :
    DegreeLE ι 1 (MvPolynomial.X i : MvPolynomial ι R) := by
  simpa [MvPolynomial.X, exponentDegree] using
    (degreeLE_monomial ι (Finsupp.single i 1) (1 : R))
...
private theorem degreeLE_map_of_monomial
    (T : Module.End R (MvPolynomial ι R)) (q d : ℕ)
    (hT : ∀ e : ι →₀ ℕ, exponentDegree ι e ≤ q →
      DegreeLE ι d (T (MvPolynomial.monomial e 1)))
    (f : MvPolynomial ι R) (hf : DegreeLE ι q f) : DegreeLE ι d (T f) := by
  rw [f.as_sum, map_sum]
  rw [degreeLE_iff_totalDegree_le]
  ...
  rw [show MvPolynomial.monomial e (MvPolynomial.coeff e f) =
    ...
    calc
      (MvPolynomial.coeff e f) • MvPolynomial.monomial e 1 =
          MvPolynomial.monomial e ((MvPolynomial.coeff e f) • (1 : R)) :=
...
private theorem basisPolynomial_degreeLE (x : L) :
    DegreeLE ι 1 (basisPolynomial R L ι b x) := by
...
private structure StageInvariant (p : ℕ) : Prop where
  stable : ∀ (q : ℕ), q ≤ p → 1 ≤ p → q < p →
    ∀ (x : L) (f : MvPolynomial ι R), DegreeLE ι q f →
      cartanStage R L ι b p x f = cartanStage R L ι b (p - 1) x f
  bounded : ∀ (q : ℕ), q ≤ p →
    ∀ (x : L) (f : MvPolynomial ι R), DegreeLE ι q f →
      DegreeLE ι (q + 1) (cartanStage R L ι b p x f)
  leading : ∀ (q : ℕ), q ≤ p →
    ∀ (x : L) (e : ι →₀ ℕ), exponentDegree ι e = q →
      DegreeLE ι q
        (cartanStage R L ι b p x (MvPolynomial.monomial e 1) -
          basisPolynomial R L ι b x * MvPolynomial.monomial e 1)
  compatible : ∀ (q : ℕ), q + 1 ≤ p →
    ∀ (x y : L) (f : MvPolynomial ι R), DegreeLE ι q f →
      cartanStage R L ι b p ⁅x, y⁆ f =
        cartanStage R L ι b p x (cartanStage R L ι b p y f) -
          cartanStage R L ι b p y (cartanStage R L ι b p x f)
...
private theorem stageInvariant (p : ℕ) : StageInvariant L ι b p := by
  ...
  | zero =>
      ...
      · intro q hq x f hf
        ...
        · intro e he
          ...
          subst e
          rw [show MvPolynomial.monomial (0 : ι →₀ ℕ) (1 : R) = 1 by simp]
          have hstage := cartanStage_apply_one R L ι b 0 x
          rw [degreeLE_iff_totalDegree_le]
          calc
            (((cartanStage R L ι b 0) x) 1).totalDegree =
                (basisPolynomial R L ι b x).totalDegree :=
  ...
  | succ p ih =>
      have hstableBasis : ∀ (i : ι) (e : ι →₀ ℕ),
          ...
          cartanStage R L ι b (p + 1) (b i) (MvPolynomial.monomial e 1) =
            cartanStage R L ι b p (b i) (MvPolynomial.monomial e 1) := by
        intro i e he
        ...
        · have hzero : e = 0 := by
            ...
            exact (Finsupp.degree_eq_zero_iff e).mp he0
          subst e
          simp only [show MvPolynomial.monomial (0 : ι →₀ ℕ) (1 : R) = 1 by simp]
        ...
        · have hp0 : p ≠ 0 := by omega
          ...
          have htail_bound : DegreeLE ι (exponentDegree ι tail)
              (MvPolynomial.monomial tail (1 : R)) := degreeLE_monomial ι tail (1 : R)
          have hinner (x : L) :
              cartanStage R L ι b p x (MvPolynomial.monomial tail (1 : R)) =
                cartanStage R L ι b (p - 1) x (MvPolynomial.monomial tail (1 : R)) :=
            ih.stable (exponentDegree ι tail) (by omega) (by omega) htail_lt
              x _ htail_bound
          have hrem : DegreeLE ι (exponentDegree ι tail)
              (cartanStage R L ι b p (b i) (MvPolynomial.monomial tail (1 : R)) -
          ...
          have hrem' : DegreeLE ι (exponentDegree ι tail)
              (cartanStage R L ι b (p - 1) (b i) (MvPolynomial.monomial tail (1 : R)) -
                MvPolynomial.monomial (Finsupp.single i 1 + tail) 1) := by
            simpa [hinner (b i)] using hrem
          have houter :
              cartanStage R L ι b p (b j)
                  (cartanStage R L ι b (p - 1) (b i)
                      (MvPolynomial.monomial tail (1 : R)) -
                    MvPolynomial.monomial (Finsupp.single i 1 + tail) 1) =
                cartanStage R L ι b (p - 1) (b j)
                  (cartanStage R L ι b (p - 1) (b i)
                      (MvPolynomial.monomial tail (1 : R)) -
                    MvPolynomial.monomial (Finsupp.single i 1 + tail) 1) :=
            ih.stable (exponentDegree ι tail) (by omega) (by omega) htail_lt
          ...
          have hpstage : cartanStage R L ι b p =
              cartanStep R L ι b (cartanStage R L ι b (p - 1)) := by
            simpa [hp0] using cartanStage_eq_step R L ι b p
          calc
            cartanStage R L ι b (p + 1) (b i) (MvPolynomial.monomial e 1) =
                (1 : R) • cartanMonomialStep R L ι b
                  (cartanStage R L ι b p) i e := by
              change cartanStep R L ι b (cartanStage R L ι b p) (b i)
                  (MvPolynomial.monomial e 1) = _
              exact cartanStep_basis_monomial R L ι b
                (cartanStage R L ι b p) i e 1
            _ = (1 : R) • cartanMonomialStep R L ι b
                (cartanStage R L ι b (p - 1)) i e := by
              apply congrArg ((1 : R) • ·)
              simp only [cartanMonomialStep, hcons]
              split
              · rfl
              · rw [hinner (b i), houter, hbracket]
            _ = cartanStage R L ι b p (b i) (MvPolynomial.monomial e 1) := by
              rw [hpstage]
              exact (cartanStep_basis_monomial R L ι b
                (cartanStage R L ι b (p - 1)) i e 1).symm
      have hstable : ∀ (q : ℕ), q ≤ p →
          ∀ (x : L) (f : MvPolynomial ι R), DegreeLE ι q f →
            cartanStage R L ι b (p + 1) x f =
              cartanStage R L ι b p x f := by
        intro q hq x f hf
        let A : L →ₗ[R] MvPolynomial ι R :=
          (LinearMap.applyₗ f).comp (cartanStage R L ι b (p + 1))
        let B : L →ₗ[R] MvPolynomial ι R :=
          (LinearMap.applyₗ f).comp (cartanStage R L ι b p)
        change A x = B x
        have hAB : A = B := by
          apply b.ext
          intro i
      ...
      have hleadingBasis : ∀ (i : ι) (e : ι →₀ ℕ),
        ...
        · have hzero : e = 0 := by
          ...
          simp only [show MvPolynomial.monomial (0 : ι →₀ ℕ) (1 : R) = 1 by simp,
            mul_one]
          have hstage := cartanStage_apply_one R L ι b (p + 1) (b i)
          have hbasis := basisPolynomial_basis R L ι b i
          rw [degreeLE_iff_totalDegree_le]
          calc
            (((cartanStage R L ι b (p + 1)) (b i)) 1 - MvPolynomial.X i).totalDegree =
                (0 : MvPolynomial ι R).totalDegree := by
              congr 1
        ...
        · let is := (Finsupp.toMultiset e).sort (· ≤ ·)
          ...
          have hlead :
              MvPolynomial.monomial (Finsupp.single i 1 + e) (1 : R) =
                MvPolynomial.X i * MvPolynomial.monomial e 1 := by
            simp [MvPolynomial.X, MvPolynomial.monomial_mul]
          have hstep :
              cartanStage R L ι b (p + 1) (b i) (MvPolynomial.monomial e 1) =
                cartanMonomialStep R L ι b (cartanStage R L ι b p) i e := by
            calc
              _ = (1 : R) • cartanMonomialStep R L ι b
                    (cartanStage R L ι b p) i e := by
                change cartanStep R L ι b (cartanStage R L ι b p) (b i)
                ...
                MvPolynomial.X i * MvPolynomial.monomial e 1 +
                  ...
                  cartanStage R L ι b p (b j)
                        (cartanStage R L ι b p (b i)
                          ...
                          MvPolynomial.monomial (Finsupp.single i 1 + tail) 1) +
                    cartanStage R L ι b p ⁅b i, b j⁆
                      (MvPolynomial.monomial tail (1 : R)) := by abel
            rw [halg]
            exact degreeLE_add ι houter hbracket
      have hleading : ∀ (q : ℕ), q ≤ p + 1 →
          ∀ (x : L) (e : ι →₀ ℕ), exponentDegree ι e = q →
            DegreeLE ι q
              (cartanStage R L ι b (p + 1) x (MvPolynomial.monomial e 1) -
                basisPolynomial R L ι b x * MvPolynomial.monomial e 1) := by
        intro q hq x e he
        let A : L →ₗ[R] MvPolynomial ι R :=
          { toFun := fun y ↦
              cartanStage R L ι b (p + 1) y (MvPolynomial.monomial e 1) -
                basisPolynomial R L ι b y * MvPolynomial.monomial e 1
            map_add' := by intro y z; simp [add_mul]; abel
            map_smul' := by intro r y; simp; module }
        change DegreeLE ι q (A x)
        apply degreeLE_linearMap_of_basis L ι b A q
        intro i
        change DegreeLE ι q
          (cartanStage R L ι b (p + 1) (b i) (MvPolynomial.monomial e 1) -
            basisPolynomial R L ι b (b i) * MvPolynomial.monomial e 1)
        rw [basisPolynomial_basis]
        rw [← he]
        exact hleadingBasis i e (he.trans_le hq)
      have hbounded : ∀ (q : ℕ), q ≤ p + 1 →
          ∀ (x : L) (f : MvPolynomial ι R), DegreeLE ι q f →
            DegreeLE ι (q + 1) (cartanStage R L ι b (p + 1) x f) := by
        intro q hq x f hf
        apply degreeLE_map_of_monomial ι
          (cartanStage R L ι b (p + 1) x) q (q + 1)
        · intro e he
          ...
          have heq :
              (cartanStage R L ι b (p + 1) x (MvPolynomial.monomial e 1) -
                    basisPolynomial R L ι b x * MvPolynomial.monomial e 1) +
                  basisPolynomial R L ι b x * MvPolynomial.monomial e 1 =
                cartanStage R L ι b (p + 1) x (MvPolynomial.monomial e 1) := by abel
          rw [heq] at hsum
          exact degreeLE_mono ι (by omega) hsum
        · exact hf
      have hcompatibleLower : ∀ (q : ℕ), q + 1 ≤ p →
          ∀ (x y : L) (f : MvPolynomial ι R), DegreeLE ι q f →
            cartanStage R L ι b (p + 1) ⁅x, y⁆ f =
              cartanStage R L ι b (p + 1) x
                  (cartanStage R L ι b (p + 1) y f) -
                cartanStage R L ι b (p + 1) y
                  (cartanStage R L ι b (p + 1) x f) := by
        intro q hq x y f hf
        ...
        have hox := hstable (q + 1) hq x
          (cartanStage R L ι b p y f) hby
        have hoy := hstable (q + 1) hq y
          (cartanStage R L ι b p x f) hbx
        rw [hxy, hx, hy, hox, hoy]
        exact ih.compatible q hq x y f hf
      have hordered : ∀ (stage : ℕ) (i : ι) (e : ι →₀ ℕ),
          (∀ j ∈ e.support, i ≤ j) →
          cartanStage R L ι b stage (b i) (MvPolynomial.monomial e 1) =
      ...
      have hrec : ∀ (a gamma : ι) (ls : List ι),
        ...
        let leadA : MvPolynomial ι R :=
          MvPolynomial.monomial (Finsupp.single a 1 + tail) 1
        let rem : MvPolynomial ι R :=
          cartanStage R L ι b p (b a) (MvPolynomial.monomial tail (1 : R)) - leadA
        have hrem : DegreeLE ι (exponentDegree ι tail) rem := by
          ...
          simpa [basisPolynomial_basis, MvPolynomial.X,
            ...
            ih.leading (exponentDegree ι tail) htail_le
              (b a) tail rfl
        have hremStable :
            cartanStage R L ι b (p + 1) (b gamma) rem =
              cartanStage R L ι b p (b gamma) rem :=
        ...
        have hleadOrdered :
            cartanStage R L ι b (p + 1) (b gamma) leadA =
              MvPolynomial.monomial
        ...
        have hstep :
            cartanStage R L ι b (p + 1) (b a) (MvPolynomial.monomial full 1) =
              cartanMonomialStep R L ι b (cartanStage R L ι b p) a full := by
          calc
            _ = (1 : R) • cartanMonomialStep R L ι b
                  (cartanStage R L ι b p) a full := by
              change cartanStep R L ι b (cartanStage R L ι b p) (b a)
                  (MvPolynomial.monomial full 1) = _
              exact cartanStep_basis_monomial R L ι b
                (cartanStage R L ι b p) a full 1
            _ = _ := by module
        ...
        have hformula :
            cartanStage R L ι b (p + 1) (b a) (MvPolynomial.monomial full 1) =
              MvPolynomial.monomial (Finsupp.single a 1 + full) 1 +
                cartanStage R L ι b p (b gamma) rem +
                cartanStage R L ι b p ⁅b a, b gamma⁆
                  (MvPolynomial.monomial tail (1 : R)) := by
          rw [hstep]
          simp only [cartanMonomialStep, hsort, hnot, if_false]
          rfl
        have hsplit :
            cartanStage R L ι b p (b a) (MvPolynomial.monomial tail (1 : R)) =
              leadA + rem := by
          dsimp only [rem]
          abel
        have hleadExp :
          ...
          rw [hfullSplit]
          abel
        change cartanStage R L ι b (p + 1) (b a)
              (MvPolynomial.monomial full 1) = _
      ...
      have hstageX : ∀ (stage : ℕ), 1 ≤ stage → ∀ i j : ι,
          cartanStage R L ι b stage (b i) (MvPolynomial.X j) =
        ...
        rw [MvPolynomial.X]
        change cartanStep R L ι b (cartanStage R L ι b k) (b i)
            (MvPolynomial.monomial (Finsupp.single j 1) 1) = _
        rw [cartanStep_basis_monomial]
        have hone (x : L) :
            cartanStage R L ι b k x (1 : MvPolynomial ι R) =
              basisPolynomial R L ι b x :=
          cartanStage_apply_one R L ι b k x
        by_cases hij : i ≤ j
      ...
      have hcompatBasis : ∀ (i j : ι) (e : ι →₀ ℕ),
          exponentDegree ι e ≤ p →
          cartanStage R L ι b (p + 1) ⁅b i, b j⁆
        ...
        · let is := (Finsupp.toMultiset e).sort (· ≤ ·)
          ...
          · by_cases hij : i = j
            ...
            · by_cases hijlt : i < j
              · have hpairI : (i :: gamma :: ls).Pairwise (· ≤ ·) := by
                ...
                have hiord := cartanStage_basis_orderedList R L ι b (p + 1)
                  i (gamma :: ls) hpair
              ...
              · have hjilt : j < i := lt_of_le_of_ne (le_of_not_gt hijlt) (Ne.symm hij)
                ...
                have hjord := cartanStage_basis_orderedList R L ι b (p + 1)
                  j (gamma :: ls) hpair
          ...
          · by_cases hjg : j ≤ gamma
            ...
            · have hgi : gamma < i := lt_of_not_ge hig
              ...
              let tail : ι →₀ ℕ := Multiset.toFinsupp (ls : Multiset ι)
              let mL : MvPolynomial ι R := MvPolynomial.monomial tail (1 : R)
              let mJ : MvPolynomial ι R :=
                MvPolynomial.monomial
                  (Multiset.toFinsupp ((gamma :: ls : List ι) : Multiset ι)) 1
              have htailDegree : exponentDegree ι tail = ls.length :=
                exponentDegree_toFinsupp ι ls
              ...
              have htailBound : DegreeLE ι (exponentDegree ι tail) mL := by
                exact degreeLE_monomial ι tail (1 : R)
              ...
              have hcommGamma : ∀ (a beta : ι), gamma < a → gamma < beta →
                  cartanStage R L ι b (p + 1) (b a)
                      (cartanStage R L ι b (p + 1) (b gamma)
                        (cartanStage R L ι b (p + 1) (b beta) mL)) =
                    cartanStage R L ι b (p + 1) (b gamma)
                        (cartanStage R L ι b (p + 1) (b a)
                          (cartanStage R L ι b (p + 1) (b beta) mL)) +
                      cartanStage R L ι b (p + 1) ⁅b a, b gamma⁆
                        (cartanStage R L ι b (p + 1) (b beta) mL) := by
                intro a beta hga hgb
                let lead : MvPolynomial ι R :=
                  MvPolynomial.monomial (Finsupp.single beta 1 + tail) 1
                let w : MvPolynomial ι R :=
                  cartanStage R L ι b (p + 1) (b beta) mL - lead
                ...
                have hcw :
                    cartanStage R L ι b (p + 1) (b a)
                        (cartanStage R L ι b (p + 1) (b gamma) w) =
                      cartanStage R L ι b (p + 1) (b gamma)
                          (cartanStage R L ι b (p + 1) (b a) w) +
                        cartanStage R L ι b (p + 1) ⁅b a, b gamma⁆ w := by
                  rw [hcw0]
                  abel
                have hleadSupport :
                ...
                have hgammaLead :
                    cartanStage R L ι b (p + 1) (b gamma) lead =
                      MvPolynomial.monomial
                ...
                have hrecLead :
                    cartanStage R L ι b (p + 1) (b a)
                        (cartanStage R L ι b (p + 1) (b gamma) lead) =
                      cartanStage R L ι b (p + 1) (b gamma)
                          (cartanStage R L ι b (p + 1) (b a) lead) +
                        cartanStage R L ι b (p + 1) ⁅b a, b gamma⁆ lead := by
                  rw [hgammaLead]
                  simpa [lead, hrestTo, toFinsupp_cons, add_assoc] using hrecLead0
                have hsplit :
                    cartanStage R L ι b (p + 1) (b beta) mL = lead + w := by
                  dsimp only [w]
                  abel
              ...
              have hformula5 : ∀ (a beta : ι), gamma < a → gamma < beta →
                  cartanStage R L ι b (p + 1) (b a)
                      (cartanStage R L ι b (p + 1) (b beta) mJ) =
                    cartanStage R L ι b (p + 1) (b gamma)
                        (cartanStage R L ι b (p + 1) (b a)
                          (cartanStage R L ι b (p + 1) (b beta) mL)) +
                      cartanStage R L ι b (p + 1) ⁅b a, b gamma⁆
                          (cartanStage R L ι b (p + 1) (b beta) mL) +
                        cartanStage R L ι b (p + 1) ⁅b beta, b gamma⁆
                            (cartanStage R L ι b (p + 1) (b a) mL) +
                          cartanStage R L ι b (p + 1)
                ...
                have hcb :
                    cartanStage R L ι b (p + 1) (b a)
                        (cartanStage R L ι b (p + 1) ⁅b beta, b gamma⁆ mL) =
                      cartanStage R L ι b (p + 1) ⁅b beta, b gamma⁆
                          (cartanStage R L ι b (p + 1) (b a) mL) +
                        cartanStage R L ι b (p + 1)
                          ⁅b a, ⁅b beta, b gamma⁆⁆ mL := by
                  ...
                  abel
                change cartanStage R L ι b (p + 1) (b a)
                    (cartanStage R L ι b (p + 1) (b beta)
                      (MvPolynomial.monomial
              ...
              have htailDiff :
                  cartanStage R L ι b (p + 1) (b i)
                      (cartanStage R L ι b (p + 1) (b j) mL) -
                    cartanStage R L ι b (p + 1) (b j)
                      (cartanStage R L ι b (p + 1) (b i) mL) =
                    cartanStage R L ι b (p + 1) ⁅b i, b j⁆ mL :=
                htailCompat.symm
              ...
              have hgammaBracket :
                  cartanStage R L ι b (p + 1) (b gamma)
                      (cartanStage R L ι b (p + 1) ⁅b i, b j⁆ mL) =
                    cartanStage R L ι b (p + 1) ⁅b i, b j⁆
                        (cartanStage R L ι b (p + 1) (b gamma) mL) +
                      cartanStage R L ι b (p + 1) ⁅b gamma, ⁅b i, b j⁆⁆ mL := by
                rw [hgammaBracket0]
                abel
              have hgammaML :
                  cartanStage R L ι b (p + 1) (b gamma) mL = mJ := by
                dsimp only [mL, mJ]
              ...
              have hjacobiAction := congrArg
                (fun x : L ↦ cartanStage R L ι b (p + 1) x mL) hjacobi
              simp only [map_add, map_sub, map_zero] at hjacobiAction
              change
                cartanStage R L ι b (p + 1) ⁅b gamma, ⁅b i, b j⁆⁆ mL +
                    cartanStage R L ι b (p + 1) ⁅b i, ⁅b j, b gamma⁆⁆ mL -
                    cartanStage R L ι b (p + 1) ⁅b j, ⁅b i, b gamma⁆⁆ mL = 0
                at hjacobiAction
              have hdiffFormula :
                  cartanStage R L ι b (p + 1) (b i)
                        (cartanStage R L ι b (p + 1) (b j) mJ) -
                      cartanStage R L ι b (p + 1) (b j)
                        (cartanStage R L ι b (p + 1) (b i) mJ) =
                    cartanStage R L ι b (p + 1) (b gamma)
                        (cartanStage R L ι b (p + 1) (b i)
                            (cartanStage R L ι b (p + 1) (b j) mL) -
                          cartanStage R L ι b (p + 1) (b j)
                            (cartanStage R L ι b (p + 1) (b i) mL)) +
                      cartanStage R L ι b (p + 1) ⁅b i, ⁅b j, b gamma⁆⁆ mL -
                        cartanStage R L ι b (p + 1) ⁅b j, ⁅b i, b gamma⁆⁆ mL := by
                rw [hi5, hj5, map_sub]
                abel
              change cartanStage R L ι b (p + 1) ⁅b i, b j⁆ mJ = _
              rw [hdiffFormula, htailDiff, hgammaBracket, hgammaML]
              rw [show
                cartanStage R L ι b (p + 1) ⁅b i, b j⁆ mJ +
                      cartanStage R L ι b (p + 1) ⁅b gamma, ⁅b i, b j⁆⁆ mL +
                      cartanStage R L ι b (p + 1) ⁅b i, ⁅b j, b gamma⁆⁆ mL -
                      cartanStage R L ι b (p + 1) ⁅b j, ⁅b i, b gamma⁆⁆ mL =
                    cartanStage R L ι b (p + 1) ⁅b i, b j⁆ mJ +
                      (cartanStage R L ι b (p + 1) ⁅b gamma, ⁅b i, b j⁆⁆ mL +
                       cartanStage R L ι b (p + 1) ⁅b i, ⁅b j, b gamma⁆⁆ mL -
                       cartanStage R L ι b (p + 1) ⁅b j, ⁅b i, b gamma⁆⁆ mL) by
      ...
      · intro q hq x y f hf
        ...
        have hb (i j : ι) :
            cartanStage R L ι b (p + 1) ⁅b i, b j⁆ f =
              cartanStage R L ι b (p + 1) (b i)
                  (cartanStage R L ι b (p + 1) (b j) f) -
                cartanStage R L ι b (p + 1) (b j)
                  (cartanStage R L ι b (p + 1) (b i) f) := by
          let A : Module.End R (MvPolynomial ι R) :=
            cartanStage R L ι b (p + 1) ⁅b i, b j⁆
          let B : Module.End R (MvPolynomial ι R) :=
            cartanStage R L ι b (p + 1) (b i) *
                cartanStage R L ι b (p + 1) (b j) -
              cartanStage R L ι b (p + 1) (b j) *
                cartanStage R L ι b (p + 1) (b i)
        ...
        let left : L →ₗ[R] L →ₗ[R] MvPolynomial ι R :=
          LinearMap.mk₂ R
            (fun x y ↦ cartanStage R L ι b (p + 1) ⁅x, y⁆ f)
            (by intros; simp [add_lie])
            (by intros; simp)
            (by intros; simp [lie_add])
            (by intros; simp)
        let right : L →ₗ[R] L →ₗ[R] MvPolynomial ι R :=
          LinearMap.mk₂ R
            (fun x y ↦
              cartanStage R L ι b (p + 1) x
                  (cartanStage R L ι b (p + 1) y f) -
            ...
            (by
              intro c m n
              simp only [map_smul]
              change c • cartanStage R L ι b (p + 1) m
                      (cartanStage R L ι b (p + 1) n f) -
                    cartanStage R L ι b (p + 1) n
                      (c • cartanStage R L ι b (p + 1) m f) = _
              rw [map_smul]
              module)
            (by intros; simp; module)
            (by
              intro c m n
              simp only [map_smul]
              change cartanStage R L ι b (p + 1) m
                      (c • cartanStage R L ι b (p + 1) n f) -
                    c • cartanStage R L ι b (p + 1) n
                      (cartanStage R L ι b (p + 1) m f) = _
              rw [map_smul]
              module)
...
private theorem cartanStage_eq_action_of_degreeLE (N : ℕ)
    (x : L) (f : MvPolynomial ι R) (hf : DegreeLE ι N f) :
    cartanStage R L ι b N x f = cartanAction R L ι b x f := by
  induction N generalizing x f with
  | zero =>
      have hmono (e : ι →₀ ℕ) (he : exponentDegree ι e ≤ 0) :
          cartanStage R L ι b 0 x (MvPolynomial.monomial e 1) =
            cartanAction R L ι b x (MvPolynomial.monomial e 1) := by
        have hb (i : ι) :
            cartanStage R L ι b 0 (b i) (MvPolynomial.monomial e 1) =
              cartanAction R L ι b (b i) (MvPolynomial.monomial e 1) := by
          have ha := cartanAction_basis_monomial R L ι b i e 1
          rw [Nat.eq_zero_of_le_zero he] at ha
          calc
            cartanStage R L ι b 0 (b i) (MvPolynomial.monomial e 1) =
                (1 : R) • cartanStage R L ι b 0 (b i)
                  (MvPolynomial.monomial e 1) := by module
            _ = cartanAction R L ι b (b i) (MvPolynomial.monomial e 1) :=
              ha.symm
        let A : L →ₗ[R] MvPolynomial ι R :=
          (LinearMap.applyₗ (MvPolynomial.monomial e 1)).comp
            (cartanStage R L ι b 0)
        let B : L →ₗ[R] MvPolynomial ι R :=
          (LinearMap.applyₗ (MvPolynomial.monomial e 1)).comp
            (cartanAction R L ι b)
        have hAB : A = B := by
          apply b.ext
          intro i
          exact hb i
        exact LinearMap.congr_fun hAB x
      exact linearMap_eq_on_degreeLE_of_monomial ι
        (cartanStage R L ι b 0 x) (cartanAction R L ι b x) 0 hmono f hf
  | succ N ih =>
      have hmono (e : ι →₀ ℕ) (he : exponentDegree ι e ≤ N + 1) :
          cartanStage R L ι b (N + 1) x (MvPolynomial.monomial e 1) =
            cartanAction R L ι b x (MvPolynomial.monomial e 1) := by
        have hb (i : ι) :
            cartanStage R L ι b (N + 1) (b i) (MvPolynomial.monomial e 1) =
              cartanAction R L ι b (b i) (MvPolynomial.monomial e 1) := by
          by_cases htop : exponentDegree ι e = N + 1
          · have ha := cartanAction_basis_monomial R L ι b i e 1
            rw [htop] at ha
            calc
              cartanStage R L ι b (N + 1) (b i)
                    (MvPolynomial.monomial e 1) =
                  (1 : R) • cartanStage R L ι b (N + 1) (b i)
                    (MvPolynomial.monomial e 1) := by module
              _ = cartanAction R L ι b (b i) (MvPolynomial.monomial e 1) :=
                ha.symm
        ...
        exact LinearMap.congr_fun hAB x
      exact linearMap_eq_on_degreeLE_of_monomial ι
        (cartanStage R L ι b (N + 1) x) (cartanAction R L ι b x)
        (N + 1) hmono f hf

private theorem cartanActionLieCompatible_commRing :
    CartanActionLieCompatible R L ι b := by
  intro x y
  apply LinearMap.ext
  intro f
  ...
  | monomial e r =>
      ...
      let m : MvPolynomial ι R := MvPolynomial.monomial e 1
      ...
      have hxBound : DegreeLE ι (d + 1)
          (cartanStage R L ι b (d + 1) x m) :=
        hstage.bounded d (by omega) x m hm
      have hyBound : DegreeLE ι (d + 1)
          (cartanStage R L ι b (d + 1) y m) :=
        hstage.bounded d (by omega) y m hm
      have hunit :
          cartanAction R L ι b ⁅x, y⁆ m =
            cartanAction R L ι b x (cartanAction R L ι b y m) -
              cartanAction R L ι b y (cartanAction R L ι b x m) := by
        calc
          cartanAction R L ι b ⁅x, y⁆ m =
              cartanStage R L ι b (d + 1) ⁅x, y⁆ m :=
            (cartanStage_eq_action_of_degreeLE L ι b (d + 1) ⁅x, y⁆ m hmN).symm
          _ = cartanStage R L ι b (d + 1) x
                  (cartanStage R L ι b (d + 1) y m) -
                cartanStage R L ι b (d + 1) y
                  (cartanStage R L ι b (d + 1) x m) :=
            hstage.compatible d (by omega) x y m hm
          _ = cartanAction R L ι b x (cartanAction R L ι b y m) -
                cartanAction R L ι b y (cartanAction R L ι b x m) := by
      ...
      rw [show MvPolynomial.monomial e r = r • m by
        ...
        calc
          MvPolynomial.monomial e r =
              MvPolynomial.monomial e (r • (1 : R)) := by
            congr
            simp
          _ = r • MvPolynomial.monomial e 1 := by rw [map_smul]]
      simp only [map_smul]
      change r • cartanAction R L ι b ⁅x, y⁆ m =
        r • (cartanAction R L ι b x (cartanAction R L ι b y m) -
          cartanAction R L ι b y (cartanAction R L ι b x m))
      rw [hunit]
  | add p q hp hq =>
      simp only [map_add, hp, hq]

/-- Ordered PBW over `R` for a Lie ring with a chosen ordered additive basis. -/
theorem freeModulePBW_commRing : PBW.FreeModulePBW R L ι b :=
  freeModulePBW_of_cartanAction_lie R L ι b
    (cartanActionLieCompatible_commRing L ι b)
