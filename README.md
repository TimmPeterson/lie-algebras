...
- For an Abelian Lie algebra, PBW is a basis-free algebra equivalence:
  ...
  $U_R(L) \simeq_R \operatorname{Sym}_R(L)$.
...
- `FreeLieDimension.dimensionSubring_succ_eq_lowerCentralSeries`: for the free Lie ring
  $F=\operatorname{FreeLie}_{\mathbb{Z}}(X)$, $\delta_n(F)=\gamma_n(F)$ in every positive degree.
...
- `lowerCentralFiltration R L` and `dimensionFiltration R L` define the lower-central and
  ...
  $\operatorname{gr}_\gamma(L)\to\operatorname{gr}_\delta(L)$.
...
| $\operatorname{gr}_\gamma(L)\to\operatorname{gr}_\delta(L)$ and its Abelian cokernel | `lowerCentralToDimensionGraded`, `DimensionGradedCokernel` | `DimensionSubring/Graded.lean` |
