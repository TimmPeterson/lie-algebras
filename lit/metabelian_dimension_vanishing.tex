...
\eqref{eq:CE-relation-Jacobi} and
	...
	Here is a completely explicit global ledger for these local identities.
	A $k$-marked row is a placed word in which the marked entry
	$\langle\rho,k\rangle$ evaluates as $\pr_{\leq k}\rho$; put
	$\langle\rho,n+1\rangle=\rho$ and
	$\langle\rho,0\rangle=0$.  In addition to the first transfer in
	\eqref{eq:CE-transfers}, the two rules involving a mark are
	\begin{align}
	...
	\end{align}
	when $v\in F_h$ is homogeneous; an index above $n+1$ is read as
	$n+1$.  The first equality is literal because
	\eqref{eq:transfer-truncation-square} says
	$[v,\pr_{\leq k}\rho]=\pr_{\leq k+h}[v,\rho]$, and the second one is
	\eqref{eq:successive-weight-differential}.  In particular, every
	marked correction in \eqref{eq:marked-transfer} is still labelled by
	the full relation $[v,\rho]\in\Rel$.

	Fix the lexicographic processing order specified above, and give every
	occurrence of a row a separate label, even when two rows have the same
	value.  This marked version is again finite: a correction in
	\eqref{eq:marked-transfer} has smaller factor number, its first summand
	has fewer unresolved adjacent pairs, and
	\eqref{eq:marked-truncation} lowers the mark.  Let $I$ be the signed
	list of initial labelled rows.  If the
	first $j$ replacements have been made, let $F_j$ be the signed
	frontier: the list of outputs produced so far which have not yet been
	used as inputs.  For a replacement $s$, write
	$\partial_{\rm led}s$ for its input minus its signed list of outputs.
	Then the global invariant is
	\begin{equation}\label{eq:prefix-ledger}
	 I-F_j=\sum_{i=1}^{j}\partial_{\rm led}s_i .
	\end{equation}
	This is an identity in the free abelian group on labelled row
	occurrences, not merely after evaluation.  It holds for $j=0$, and the
	induction step is immediate: $s_{j+1}$ removes its input occurrence
	from $F_j$ and inserts precisely its output occurrences.  Termination
	therefore gives
	\begin{equation}\label{eq:complete-ledger}
	 I-F_N=\sum_{i=1}^{N}\partial_{\rm led}s_i.
	\end{equation}
	Every internal occurrence is, by construction, one output and one
	later input, with opposite signs.  Thus
	\eqref{eq:complete-ledger} is the promised finite integral Stokes sum;
	it does not assume any statement about its external boundary.  The
	critical-pair calculation above shows in addition that changing the
	chosen processing order changes only paired internal occurrences.

After applying the one-factor, weight-$(n+1)$ projection, all
off-diagonal cells are silent.  Indeed, Jacobi expresses a primitive
bracket tree as an integral sum of combs.  A nonzero metabelian comb has
at most one input in $M$.  If this input is the relation component
$\rho_k$, all other inputs are in $F_1$ and the output has weight
$k+p-1$; it is $n+1$ precisely when $p=m_k$.  If the unique $M$-input
is ordinary, all higher components of the relation bracket trivially
with it, so the comb is unchanged when the weight-one relation component
is replaced by the full relation; its image in $C$ is zero.  Two
$M$-inputs give zero, and a spectator is not primitive.  If every input
has weight one, reaching weight $n+1$ requires $p=n+1$; replacing the
weight-one relation component by the full relation adds only terms above
the nilpotency class, so this is again a full commutator relation.
Larger factor numbers cannot reach weight $n+1$.  Thus the
diagonal cells, the oriented factor-two cell, and the terminal
factor-one edge are the complete external boundary after projection to
$C$.

	Apply the evaluation map to \eqref{eq:complete-ledger}, and then apply
	the one-factor,
	weight-$(n+1)$ projection followed by $F_{n+1}\to C$.  By the preceding
	Every $\partial_{\rm led}s_i$ evaluates to zero because its defining
	replacement is one of the literal identities above.  By the preceding
	classification, the complete external boundary, including signs, is
	\begin{center}
	\begin{tabular}{c|c}
	...
	\end{tabular}
	\end{center}
	For each labelled diagonal occurrence, equations
	\eqref{eq:cell-horizontal-edge} and
	\eqref{eq:cell-vertical-edge} give the two entries in the corresponding
	row of this table, with opposite boundary orientations.  Summing over
	the occurrences comprising $\chi_k$ gives exactly
	$-\Phi_k(f_k\chi_k)+T_k(\partial\chi_k)$ by linearity; thus neither the
	coefficients nor the signs in the table are inferred from the desired
	conclusion.
	All factor-one full-relation occurrences are retained in the terminal
	frontier.  They are not projected separately: after the horizontal and
	quadratic edges have been read, their signed terminal contribution is
	precisely the difference defined as $\varepsilon$ in
	\eqref{eq:terminal-definition}.  The homogeneous full commutator leaves
	classified above map to zero in $C$, and all other external families are
	silent.  Thus no relation tail is being declared a relation at this
	point.
	Consequently \eqref{eq:complete-ledger}, rather than an unproved
	pictorial cancellation, gives the literal equality
	...
	\eqref{eq:terminal-discrepancy}.  Notice that this conclusion does
	\emph{not} say that a separated terminal tail is a relation.  It
	identifies that tail with actual boundary edges in the full row complex.
...
\begin{proof}
Apply Lemma~\ref{lem:closed-square} to $\Theta=\theta$.  A primitive
factor-lowering term is an iterated bracket tree.  Jacobi writes it
integrally as a sum of combs, and a nonzero metabelian comb has at most
one input in $M=F_{\geq2}$.  If that input is the $F_k$-component of the
relation differential, all other inputs lie in $F_1$; reaching weight
$n+1$ forces exactly $n+1-k=m_k-1$ of them.  The resulting comb is
\eqref{eq:cell-horizontal-edge}.  If the unique $M$-input is ordinary,
replacing the weight-one relation input by the full relation changes
nothing, since every higher relation component then meets the ordinary
$M$-input and vanishes.  It is therefore a full commutator relation.
Two $M$-inputs give zero, a spectator leaves at least two PBW factors,
factor number $n+1$ gives a full commutator of a weight-one relation,
and larger factor numbers cannot reach weight $n+1$.  This proves that
the displayed intermediate combs and the factor-two block are the
complete external primitive boundary.

At factor two, opposite placements must be retained.  The cycle
$\chi_n$ splits into the four Smith families
\[
 P_1\otimes P_0,\quad P_1\otimes Q_0,\quad
 Q_1\otimes P_0,\quad Q_1\otimes Q_0.
\]
Proposition~\ref{prop:quadratic-block} identifies their oriented
primitive coefficient with
$-\eta_n(L_1\Sym^2(\pi_n)[\chi_n])$.  The only remaining external edge
is the terminal discrepancy of Lemma~\ref{lem:closed-square}, and it is
zero.  Since \eqref{eq:primitive-witness} identifies the primitive
weight-$(n+1)$ coefficient with $\zeta\widetilde z$, summing the external
edges gives \eqref{eq:PBW-normal-form}.
\end{proof}
