...
Write
$\theta=\sum_{\alpha\in A}c_\alpha\rho_\alpha u_\alpha$ with $A$
finite, giving every occurrence $\alpha$ of a full relation its own
label; angle brackets below remember this occurrence label, not merely
the value of $\rho_\alpha$.  Fix homogeneous PBW
...
Here is an indexed form of the collector.  Let $\mathcal P$ be the
free $\R$-module on labelled placed rows of weight at most four, and
let $\operatorname{ev}:\mathcal P\to\U(F)/\U(F)_{\ge5}$ evaluate a
row, where $\U(F)_{\ge5}$ is the span of PBW monomials of total weight
at least five.  PBW-expand each $u_\alpha$, retaining the relation
mark; this gives an initial frontier $C_0\in\mathcal P$.  If step $j$ selects the occurrence
$r_j$ with coefficient $b_j$ and its literal rule has output
$O_j\in\mathcal P$, give every occurrence in $O_j$ a fresh child label
recording $r_j$ as parent, and set
\begin{equation}
 C_{j+1}=C_j-b_jr_j+b_jO_j,\qquad
 C_0-C_N=\sum_{j=0}^{N-1}b_j(r_j-O_j).                 \label{eq:frontier}
\end{equation}
The second equality is induction on $N$, and
$\operatorname{ev}(r_j-O_j)=0$ for every $j$.  Thus
$\operatorname{ev}(C_N)=\operatorname{ev}(C_0)=\theta$ in the retained
weights.  At the one-factor frontier, the cut descendants belonging to one
marked relation occurrence are first reassembled to the full relation
and only then set equal to zero in $K$.

For $p=3,2$, let $A_p$ be the finite set of distinctly labelled marked
$p$-factor occurrences when their factor-preserving symbols are taken,
including every correction arriving from the preceding queue; write
$c_\nu$ for the coefficient of occurrence $\nu$.  After the prescribed
placement and projection, define
\[
 q_3(\nu)=(\rho_\nu^{[2]};x_\nu y_\nu)\in K_3(S)_1,
 \qquad q_2(\nu)=(\rho_\nu^{[3]};z_\nu)\in K_2(R)_1,
 \qquad
 \Xi_p=\sum_{\nu\in A_p}c_\nu q_p(\nu).
\]
The truncations lie in the asserted kernels because $\rho_\nu$ maps to
zero and the omitted pieces have weight at least three, respectively
four.  Let $\sigma_p$ denote the $p$-factor PBW coefficient in weights
at most four.  PBW identifies the factor-number symbol with a symmetric
algebra.  Thus the factor-preserving output of occurrence $\nu$ is
$\partial q_p(\nu)$ and every other output enters a lower queue.
Induction in \eqref{eq:frontier} gives the literal identities
\begin{equation}
 \sigma_p(\theta)=\sum_{\nu\in A_p}c_\nu\partial q_p(\nu)
                 =\partial\Xi_p\qquad(p=3,2).           \label{eq:symbol}
\end{equation}
Total bracket weight is preserved throughout, and PBW gives
\begin{equation}
                         \aug(F)^5=\U(F)_{\ge5}.         \label{eq:PBWweight}
\end{equation}
...
reassembles a full relation.

We can now write the promised indexed readout.  Partition the terminal
one-factor occurrences into $A_1^{\rm rel}$ and $A_1^{\rm exp}$,
according as they are reassembled full relations $\tau_\mu$ or
unmarked primitive terms $z_\mu$; retain their coefficients $c_\mu$,
and write $\bar z_\mu$ for the image in $K$.
Taking the one-factor PBW coefficient in \eqref{eq:frontier}, and then
using \eqref{eq:witness}--\eqref{eq:PBWweight}, gives the literal equality
\[
 f=\sigma_1(\theta)=\sigma_1(\operatorname{ev}C_N)=
 \sum_{\mu\in A_1^{\rm rel}}c_\mu\tau_\mu+
 \sum_{\mu\in A_1^{\rm exp}}c_\mu z_\mu .
\]
The first sum maps to zero in $K$.  Since fresh labels retain their
parent, the replacement list is a finite rooted forest.  Table
\eqref{eq:table} defines a source map
$s:A_1^{\rm exp}\to A_3\sqcup A_2$: it sends a terminal tooth to the
unique queue occurrence where its $211$ or $31$ edge was exposed.
The occurrencewise computations above, including the labelwise $22$
closure, say precisely that for $\nu\in A_p$,
\begin{equation}
 \sum_{\substack{\mu\in A_1^{\rm exp}\\s(\mu)=\nu}}
 c_\mu\chi(\bar z_\mu)
       =c_\nu(dT_p)(q_p(\nu))\qquad(p=3,2).             \label{eq:fibre}
\end{equation}
Summing the disjoint finite fibres in \eqref{eq:fibre}, and using the
definitions of $\Xi_p$, gives the completely indexed formula
\begin{align}
 \chi(a)
 &=\sum_{\mu\in A_1^{\rm exp}}c_\mu\chi(\bar z_\mu)\notag\\
 &=\sum_{\nu\in A_3}c_\nu(dT_3)(q_3(\nu))
   +\sum_{\nu\in A_2}c_\nu(dT_2)(q_2(\nu))\notag\\
 &=dT_3(\Xi_3)+dT_2(\Xi_2).                            \label{eq:readout}
\end{align}
$A_1^{\rm exp},A_3,A_2$ are finite sets of distinct occurrence labels.
Thus \eqref{eq:frontier}, proved by induction on the finite replacement
list, is the only global bookkeeping step; no confluence assertion or
unindexed cancellation is used.

\pagebreak[3]
For comparison, all possible adjacent
overlaps are also closed:
